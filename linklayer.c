...
/*A função é responsável por abrir a conexão usando parâmetros “port” definidos na linkLayer. 
Inicialmente colocamos os connection parameters em “ll” pois permitem usar o ladder  ao longo de todo o programa. 
Posteriormente procedemos à verificação da abertura da porta que, caso não ocorra, é necessário chamar a função llopen() novamente. 
Também é nesta fase do programa que ocorre o pedido do BaudRate.
Utilizámos a variável role para definir se se trata do “TRANSMITTER” ou “Receiver”. 
No primeiro caso definimos um vetor onde colocamos a trama a enviar para criar a ligação que será comprovada pela recessão de flags 
com funcionalidades distintas.  
Após a definição de todos os parâmetros e variáveis procedemos à implementação de um código sequencial que limita a 3 retransmissões da trama.
Usamos a função statet_machine() para ler o header e se receber a trama “UA”, lemos a última FLAG. Caso haja erro, retransmitimos. 
No segundo caso, o receiver vai estar sempre apto a ler o próximo independentemente da ocorrência de erros. 
Ao ser verificada a trama de recessão “Set” podemos concluir que o header foi bem recebido e procede à leitura da informação até que a 
última Flag seja lida, reenviando a confirmação caso esta esteja correta. Tal deteção de erros é realizada pelo BCC1.￼*/
...
Enviamos uma trama com controlo 0 ou 1 e recebemos uma trama RR ou REJ, com controlo 1 ou 0, respetivamente.
Depois definimos o bit de paridade da informação a enviar, e fazemos byte stuffing para uma variável stuffed[]. 
Depois, escrevemos os dados e esperamos a confirmação.
Na receção, se for detetado algum erro, ou recebermos REJ, temos de reenviar a trama. 
Caso recebamos o RR, sem erros, a função foi bem concluída.*/
...
A sua funcionalidade é ler a informação recebida onde também realiza verificações de erros na data utilizando o BCC2.
Inicialmente esta função permanece num ciclo enquanto espera pela receção da trama que a função read() irá retornar.
Após esta recessão irá ser descodificada a informação presente na trama e aguarda a receção da última flag,
a primeira que indica o início da receção e a segunda define o final. Também verifica o comando SET e envia a trama UA
Enviamos uma trama de controlo RR com o número 1 em caso de sucesso da transmissão, isto é, 
sem erros e permite então a saída do ciclo. No caso de existirem erros é enviada a trama REJ com esta informação para que a 
informação seja enviada de novo.*/
...
/*No final é necessário encerrar a conexão. Para isto é necessário verificar, como anteriormente, se se trata do “Transmitter” ou “Receiver”. 
O transmitter verifica o número de retransmissões. Verifica se recebemos o header corretamente e ao confirmar a recessão da última Flag 
podemos proceder à escrita dessa confirmação. Para o Receiver verificamos também a receção correta do header, Disc, e da última trama, UA. 
Após todas as verificações descritas encerramos a conexão e imprimimos as estatísticas do programa. */
